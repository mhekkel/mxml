...
The class :cpp:class:`zeem::node` is the base class for all classes in the DOM API. The class is not copy constructable and subclasses use move semantics to offer a simple API while still being memory and performance efficient. Nodes can have siblings and a parent but no children.

The class :cpp:class:`zeem::element` is the main class, it implements a full XML node with child nodes and attributes. The children are stored as a linked list and same goes for the attributes.

The class :cpp:class:`zeem::text` contains the text between XML elements. A :cpp:class:`zeem::cdata` class is derived from :cpp:class:`zeem::text` and other possible child nodes for an XML element are :cpp:class:`zeem::processing_instruction` and :cpp:class:`zeem::comment`.

XML elements also contain attributes, stored in the :cpp:class:`zeem::attribute` class. Namespace information is stored in these attributes as well. Attributes support structured binding, so the following works:

.. code-block:: cpp

    zeem::attribute a("x", "1");
    auto& [name, value] = a; // name == "x", value == "1"

Input and output
--------------------------------------

The class :cpp:class:`zeem::document` looks very much like a :cpp:class:`zeem::element` but can only contain one child and no attributes. This class can load from and write to files.

streaming I/O
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

You can use std::iostream to read and write :cpp:class:`zeem::document` objects. Reading is as simple as:
...
This will not validate the XML using the DTD by default. If you do want to validate and process the DTD, you have to specify where to find this DTD and other external entities. You can either use :cpp::func:`zeem::document::set_base_dir` or you can specify an entity_loader using :cpp::func:`zeem::document::set_entity_loader`
...
When we want to see the `&hello;` entity replaced with `'Hello, world!'` as specified in the DTD, we need to provide a way to load this DTD. To do this, look at the following code. Of course, in this example a simple call to :cpp::func:`zeem::document::set_base_dir` would have been sufficient.
...
What happens during serialization is deconstruction of structured data types into parts that can be converted into text strings. For this final conversion there are zeem::value_serializer helper classes. zeem::value_serializer is a template and specializations for the default types are given in `<zeem/serializer.ixx>`. You can create your own specializations for this class for custom data types, look at the one for `std::chrono::system_clock::time_point` for inspiration.

enums
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

For conversion of enum's you can use the `zeem::value_serializer` specialization for enums:

.. code-block:: cpp

    enum class MyEnum { FOO, BAR };
    zeem::value_serializer<MyEnum>::init({
        { MyEnum::FOO, "foo" },
        { MyEnum::BAR, "bar" }
    });
...
ZEEM comes with a [XPath 1.0](http://www.w3.org/TR/xpath/) implementation. You can use this to locate elements in a DOM tree easily. For a complete description of the XPath specification you should read the documentation at e.g. http://www.w3.org/TR/xpath/ or https://www.w3schools.com/xml/xpath_intro.asp.

The way it works in ZEEM is that you can call `find()` on an :cpp:class:`zeem::element` object and it will return a `zeem::element_set` object which is actually a `std::list` of :cpp:class:`zeem::element` pointers of the elements that conform to the specification in XPath passed as parameter to `find()`. An alternative method `find_first()` can be used to return only the first element.

An example where we look for the first person in our test file with the lastname Jones:

.. code-block:: cpp

    zeem::element* jones = doc.child()->find_first("//person[lastname='Jones']");
